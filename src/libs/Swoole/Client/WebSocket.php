...
class WebSocket
{
     ...
     */
    function __construct($host, $port = 80, $path = '/', $auth_header = null)
    {
        if (empty($host)) {
    ...
    function doHandShake($headerBuffer)
    {
        ...
        if (empty($sec_websocket_accept)) {
            $sec_websocket_accept = $header['sec-websocket-accept'];
        }

        if (!isset($sec_websocket_accept)) {
            $this->disconnect();
            return false;
        }
        if ($sec_websocket_accept != base64_encode(pack('H*', sha1($this->key . self::GUID)))) {
            $this->disconnect();
            return false;
        }
        $this->handshake = true;
    ...
    final protected function createHeader()
    {
        ...
        $header_websocket = "GET {$this->path} HTTP/1.1" . "\r\n" .
            "Origin: null" . "\r\n" .
            "Host: {$host}:{$this->port}" . "\r\n" .
        ...
        if ($this->auth_header != null) {
            $header_websocket .= $this->auth_header;
        }
        $header_websocket .= "\r\n";
        //var_dump($header_websocket);
        return $header_websocket;
    }
